...
app.config(function($stateProvider, $urlRouterProvider) {
  ...
  $stateProvider.state({ name: 'home', url: '/', controller: function() { }, template: '<div style="border:1px solid lime;padding:8px"><h3>Features</h3><li>backbutton</li><li> forwardbutton</li><li> deeplinking</li><li>bookmarking</li><li>history</h1><li>send link (to helpdesk) </li><h3>Missing</h3>hash namespaces, well known deeplinks</div></div>'});
  ...
  $stateProvider.state({ name: 'page.foo.id', url: '/:id', controller: function($state) {console.log("REST call for id",$state.params.id) }, template: '<div style="border:1px solid blue"><h3>Template 3</h3>Try to reload the page.<div style="font-size:20px">$state.params.id: {{$state.params.id}}</div>'});
});
